...
contract TestU2UV3Router is IU2UV3SwapCallback {
    ...
    function U2UV3SwapCallback(
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid =
                amount0Delta > 0 ? IU2UV3Pool(msg.sender).token0() : IU2UV3Pool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0 ? amount0Delta : amount1Delta;

            bool zeroForOne = tokenToBePaid == IU2UV3Pool(pools[0]).token1();
            IU2UV3Pool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
            ...
            if (amount0Delta > 0) {
                IERC20Minimal(IU2UV3Pool(msg.sender).token0()).transferFrom(
                    payer,
                    msg.sender,
                    uint256(amount0Delta)
                );
            } else {
                IERC20Minimal(IU2UV3Pool(msg.sender).token1()).transferFrom(
                    payer,
                    msg.sender,
